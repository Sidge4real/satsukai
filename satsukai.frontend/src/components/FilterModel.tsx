...
interface ITuneModal{
    isOpen: boolean;
    onClose: () => void;
    categories: IPostFilters[];
    onApplyFilter: (filter: string | null) => void;
    selectedFilter: string | null;
  }
  
  const TuneModal = ({ isOpen, onClose, categories, onApplyFilter, selectedFilter } : ITuneModal) => {
    if (!isOpen) {
      return null;
    }
        ...
        <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          ...
          >  
            ...
            {categories && categories.map((filter) => (
              <div
                key={filter.id}
                className={`bg-opacity-100 rounded-md cursor-pointer transition-transform transform hover:bg-gray-100 ${!selectedFilter && filter.name == "Show all" ? "bg-gray-100" : ""} ${selectedFilter == filter.name && "bg-gray-100"} p-3 w-full`}
                onClick={() => onApplyFilter(filter.name)}
              >
